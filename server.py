...
def rerank(query: str, documents: List[str]) -> Optional[List[int]]:
    ...
    try:
        ...
        d = json.loads(urllib.request.urlopen(req, timeout=30).read())  # 重排超时收紧
...
def _search_table(db, table: str, query: str, limit: int):
    ...
    if good:
        ...
        for expr, weight, kind in ((" ".join('"%s"' % w for w in terms), 10.0, "jieba"),
            ...
            try:
                rows = db.execute(
                    f"SELECT path, bm25({jtable}, {BM25_WEIGHTS[jtable]}) AS rank FROM {jtable} "
...
def related(path: str, limit: int = 5) -> list:
    ...
    with closing(get_db()) as db:
        ...
        if refs:
            ph = ",".join("?" * len(refs))
            for r in db.execute(
                    f"SELECT DISTINCT page_path FROM attachment_links WHERE att_path IN ({ph}) AND page_path != ?",
                    refs + [rel]):
